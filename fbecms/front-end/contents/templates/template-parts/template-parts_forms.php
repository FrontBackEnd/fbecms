		...
		if(checkPermission($_SESSION['role'], 'add', dirname(__FILE__))){
			...
			echo '<h1 class="text-center">Add '.$name.'</h1>
				  <form method="POST" action="">
					<input type="hidden" name="data[template_id]" value="'.$_POST['template_id'].'">
				...
				if(isset($row) && !empty($row)){
					...
					echo '<h1 class="text-center">Edit '.$name.'</h1>
						  ...
						  <input type="hidden" name="data['.$chain.']" value="'.$id.'">
							...
							<div class="form-group">
								<label for="start_tag">Start tag</label>
								<input id="start_tag" type="text" value="'.htmlspecialchars_decode($row['start_tag']).'" name="data[start_tag]" class="form-control" >
							</div>
							
							<div class="form-group">
								<label for="end_tag">End tag</label>
								<input id="end_tag" type="text" value="'.htmlspecialchars_decode($row['end_tag']).'" name="data[end_tag]" class="form-control" >
							</div>
