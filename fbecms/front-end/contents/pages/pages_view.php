...
if(checkPermission($_SESSION['role'], 'view', dirname(__FILE__))){
	...
	if(isset($_POST['action']) && !empty($_POST['action'])) include($next_file."_forms.php");
		...
		echo'<h1 class="text-center">Pages</h1>
			...
			foreach($$table AS $row){
				...
				if(isset($templates) && !empty($templates)){
					if(isset($row['template_id']) && !empty($row['template_id'])){
						foreach($templates AS $key => $name) if($row['template_id'] == $key) echo '<td>'.$name.'</td>';
					}else echo '<td></td>';
