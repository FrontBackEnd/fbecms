					...
					echo '<h1 class="text-center">Edit '.$name.'</h1>
						  ...
						  <input type="hidden" name="data['.$chain.']" value="'.$id.'">
							...
							<div class="POST" action>
								<label for="title">Menu style</label>
								<select name="data[style]" class="form-control">';
					
								foreach($menu_style AS $key => $style){
									if($key == $row['style']) $sel = ' selected="selected"';
									else $sel = '';
									
									echo '<option value="'.$key.'"'.$sel.'>'.$style.'</option>';
								}
