...
switch($_POST[$save_chain]){
	...
	case 'ADD':
		...
		if(checkPermission($_SESSION['role'], 'add', dirname(__FILE__))){
			...
			$data = $_POST['data'];
			$data['tutorial_url'] = getProperUrl($data['title']);
			...
			if(mysqli_query($conn,$sql)){
				...
				if(isset($_FILES["img"]["name"]) && !empty($_FILES["img"]["name"])){
					...
					ImgCreation($_FILES, $id, $data['tutorial_url'], $table, $next_file);
				}
	...
	case 'UPDATE':
		...
		if(checkPermission($_SESSION['role'], 'update', dirname(__FILE__))){
			...
			if(isset($_FILES["img"]["name"]) && !empty($_FILES["img"]["name"])){
				ImgCreation($_FILES, $update_id, $data['tutorial_url'], $table, $next_file);
			}
